...
# -*- coding: utf-8 -*-
#
#  arfedoracontrolcenter_fedora.py
#  
...
if_true_skip         = True # skip to fix installer
if_false_skip        = True
...
all_package = ["arfedora-control-center"]
...
class Plugin(BasePlugin):
    ...
    def install(self):
        ...
        if os.path.isfile("/etc/yum.repos.d/_copr_youssefmsourani-arcontrolcenter.repo"):
            commands = ["dnf install {} -y --best".format(to_install)]
        else:
            commands = ["dnf copr enable youssefmsourani/arcontrolcenter -y","dnf install {} -y --best".format(to_install)]
